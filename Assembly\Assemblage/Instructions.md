This file contains instructions on how to put together this device, describes what the different connectors that are circled and explains how to correct the mistaque mentionned before.

Ce fichier contien des instructions sur l'assemblage de cet apareil, décris les différents connecteurs entourés et montre comment corriger l'erreur évoqué sur la première page.
...
- The connector circled in red is to connect the bottom_pcb. / Le connecteur entouré en rouge sert à connecter le circuit du bas.
- The connector circledd in orange is to connect the 3,3V regulator (based on ADAFRUIT's design). / Le connecteur entouré en orange sert à connecter le régulateur 3,3V (basé sur le design d'ADAFRUIT). 
- The connector circled in yellow is the second micro sd card slot and cannot be used alongside the first one. / Le connecteur entouré en jaune est le second slot micro sd et ne peut pas être utilisé avec le premier simultanément.
- The cyan and blue lines represents the two wires to solder and fix the i²c bus and are mandatory. / Les lignes cyan et bleu représentes les deux fils à souder pour réparer le  bus i²c et obligatoires.
